...
import (
	...
	"github.com/coollabsio/cli-coolify/internal/version"
	"github.com/sirupsen/logrus"
	"github.com/spf13/viper"
)

// Package config provides a reuseable struct that holds configuration, http client and other common functions shared by all the commands.

var DefaultConfigDirectory string = xdg.ConfigHome
...
func (c *Coolify) ConfigureClient() error {
	...
	// Add token to all requests via request editor
	requestEditor := func(ctx context.Context, req *http.Request) error {
		req.Header.Set("Authorization", "Bearer "+c.Config.Token)
		return nil
	}

	client, err := client.NewClientWithResponses(withApiPrefix, client.WithRequestEditorFn(requestEditor))
	if err != nil {
