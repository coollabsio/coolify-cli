...
func TestDestinationService_CRUDAndServerPaths(t *testing.T) {
	...
	svc := NewDestinationService(api.NewClient(server.URL, "token"))
	_, _ = svc.List(context.Background())
	_, _ = svc.ListByServer(context.Background(), "server/id")
	_, _ = svc.Get(context.Background(), "destination/id")
	_, _ = svc.CreateForServer(context.Background(), "server/id", models.DestinationCreateRequest{Name: "public", Network: "coolify", Type: "standalone"})
	_ = svc.Delete(context.Background(), "destination/id")
...
func TestCloudTokenService_CRUDAndValidate(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requests <- observed{r.Method, r.URL.EscapedPath()}
		switch r.Method {
		case http.MethodPost, http.MethodPatch:
			var body map[string]any
			assert.NoError(t, json.NewDecoder(r.Body).Decode(&body))
			if r.URL.Path == "/api/v1/cloud-tokens" {
	...
	svc := NewCloudTokenService(api.NewClient(server.URL, "token"))
	_, _ = svc.List(context.Background())
	_, _ = svc.Get(context.Background(), "token/id")
	_, _ = svc.Create(context.Background(), models.CloudTokenCreateRequest{Provider: models.CloudProviderHetzner, Token: "secret", Name: "primary"})
	_, _ = svc.Update(context.Background(), "token/id", models.CloudTokenUpdateRequest{Name: "renamed"})
	_, _ = svc.Validate(context.Background(), "token/id")
	_ = svc.Delete(context.Background(), "token/id")
