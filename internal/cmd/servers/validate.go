...
func initialValidateModel(uuid string, coolify config.Getter, ctx context.Context) validateModel {
	s := spinner.New()
...
func (m validateModel) validateServer() tea.Msg {
	...
	parsedResponse, err := client.ParseValidateServerByUuidResponse(server)
	if err != nil {
