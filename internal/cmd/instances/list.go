...
func newFilterableListModel(instances []config.Instance, sensitive bool, initialFilter string, deleteHandler func(tui.FilterableItem) error) *filterableListModel {
...
func filterInstances(instances []config.Instance, filter string) []config.Instance {
	if filter == "" {
		return instances
	}

	filtered := make([]config.Instance, 0)
...
func redactTokens(instances []config.Instance) []config.Instance {
	redacted := make([]config.Instance, len(instances))
	for i, instance := range instances {
