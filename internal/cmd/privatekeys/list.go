...
func newFilterableListModel(keys []client.PrivateKey, filter string) *filterableListModel {
...
func wrapKeys(keys []client.PrivateKey) []tui.FilterableItem {
...
func (c *cliPrivateKeys) handleDelete(item tui.FilterableItem) error {
	...
	parsedResponse, err := client.ParseDeletePrivateKeyByUuidResponse(deleteReq)
	if err != nil {
...
`),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			parsedResponse, err := client.ParseListPrivateKeysResponse(response)
			if err != nil {
				return fmt.Errorf("failed to parse response: %w", err)
			}
			...
			if format == "json" {
				...
				if !showSensitive {
					...
					redactedKeys := make([]client.PrivateKey, len(*parsedResponse.JSON200))
					for i, key := range *parsedResponse.JSON200 {
						redactedKeys[i] = key
						redactedKeys[i].PrivateKey = &config.Redacted
						redactedKeys[i].PublicKey = &config.Redacted
					}
					keys = redactedKeys
				}
