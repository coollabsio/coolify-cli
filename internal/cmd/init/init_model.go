...
func newInitModel(result chan<- []config.Instance) initModel {
	...
	return initModel{
		instances:     make([]config.Instance, 0),
		focus:         0,
		result:        result,
		step:          0,
		...
		case key.Matches(msg, m.keys.Enter):
			...
			case 2:
				...
				if m.useSelfHost {
					...
					if m.useCloud {
						m.instances = append(m.instances, config.Instance{
							Name:    "cloud",
							Default: true,
