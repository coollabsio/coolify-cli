...
// NewCloneCommand clones a one-click service to a destination.
func NewCloneCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Clone a service to a destination",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resp, err := svcservice.NewService(client).Clone(cmd.Context(), args[0], req)
			if err != nil {
				return err
			}
...
func NewRunStorageBackupCommand() *cobra.Command {
	return &cobra.Command{
		...
		Short: "Run a service volume backup schedule now",
		RunE: func(cmd *cobra.Command, args []string) error {
			client, err := cli.GetAPIClient(cmd)
			if err != nil {
				return err
			}
			resp, err := svcservice.NewService(client).RunStorageBackup(cmd.Context(), args[0], args[1])
			if err != nil {
				return err
			}
			return printAny(cmd, resp)
