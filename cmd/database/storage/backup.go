...
func NewSetBackupScheduleCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "set-backup <db_uuid> <storage_uuid>",
		Args:  cli.ExactArgs(2, "<db_uuid> <storage_uuid>"),
		Short: "Create or replace database storage volume backup schedule",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resp, err := service.NewDatabaseService(client).SetStorageBackupSchedule(cmd.Context(), args[0], args[1], req)
			if err != nil {
				return err
			}
...
func NewDeleteBackupScheduleCommand() *cobra.Command {
	return &cobra.Command{
		Use:   "delete-backup <db_uuid> <storage_uuid>",
		Args:  cli.ExactArgs(2, "<db_uuid> <storage_uuid>"),
		Short: "Delete database storage volume backup schedule",
		RunE: func(cmd *cobra.Command, args []string) error {
			client, err := cli.GetAPIClient(cmd)
			if err != nil {
				return err
			}
			if err := service.NewDatabaseService(client).DeleteStorageBackupSchedule(cmd.Context(), args[0], args[1]); err != nil {
				return err
			}
...
func NewRunBackupCommand() *cobra.Command {
	return &cobra.Command{
		Use:   "run-backup <db_uuid> <storage_uuid>",
		Args:  cli.ExactArgs(2, "<db_uuid> <storage_uuid>"),
		Short: "Run database storage volume backup now",
		RunE: func(cmd *cobra.Command, args []string) error {
			client, err := cli.GetAPIClient(cmd)
			if err != nil {
				return err
